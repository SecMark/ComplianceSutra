...
function CoAccount({ handleClose }) {
   ...
   useEffect(() => {
      const accountInfo = state && state.taskReport && state.taskReport.coAccountInfo &&
   ...
   useEffect(() => {
      const updateStatus = state && state.taskReport && state.taskReport.coAccountUpdStatus
   ...
   const closeMobile = (data, action) => {
      ...
      if (action == 1) {
         const licenseIDgrpStr = data.selectedLicenseArray.length > 0 ? data.selectedLicenseArray.join(",") : "";
         dispatch(
      ...
      <div className="co-account ">
         {!isMobile && <div id="drawerParent" className="">
            <div id="drawerChild" className="sideBarFixed">
               {licenseDrawerHideShow && <Drawer fields={fields}
                  ...
                  <div className="col-9">
                     <div className="acc-div">
                        <div className="licences-toggle">
                           <p className="normaltext">COMPLIANCE SUTRA Expert &nbsp;<span className="d-none d-sm-block"> License Review</span> <span className="review">{flagCount}</span></p>
                           <p className="normaltext d-block d-sm-none"> License Review </p>
                        </div>
                        {isSliderCheck && isMobile && <p style={{ cursor: "pointer", opacity: isSliderCheck > 0 ? 1 : 0.7, }} onClick={() => onAddLicenseLabelClick()} className="add-remove">Add/remove licenses</p>}
                        {isSliderCheck && !isMobile && <p style={{ cursor: "pointer", opacity: isSliderCheck > 0 ? 1 : 0.7, }} onClick={() => onAddLicenseLabelClick()} className="add-remove">Add/remove licenses</p>}
                     </div>
                  </div>
                  <div className="col-3">
