    ...
    <div className="row co-setting">
      ...
      <div className="col-4 left-fixed ">
        <div className="side-bar-outer">
          <SideBarInputControl />
          <SettingSideBar activeTabKey={selectedTabKey} handleTabChange={(key) => handleTabChange(key)} />
        </div>
      </div>
      {!isMobile && <div className="col-12 padding-left">
        <img className="right-bg d-none d-sm-block" src={Cobg} alt="" />
        {state.adminMenu.activeTab === "personal" && <CoPersonal activeTabKey={selectedTabKey} />}
        {state.adminMenu.activeTab === "company" && <CoCompany activeTabKey={selectedTabKey} />}
        ...
        {showHB === false &&
          <div className=" d-block d-sm-none pad-ms pr-4">
            <div className="d-flex">
              <div className="w-25" style={{ cursor: "pointer" }} onClick={() => { onHBMenu() }}><img src={togglemobile} alt="toggle mobile" /></div>
      ...
      {isMobile && <MobileSettingSideBar showHB={showHB} setShowHBMenu={setShowHBMenu} />}
    </div>
  );
}
