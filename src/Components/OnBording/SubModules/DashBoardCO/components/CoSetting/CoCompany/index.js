...
function CoManagment({ handleClose }) {
  ...
  const initialDispatch = () => {
    ...
    setCompanyDetails([]);
    dispatch(coActions.getEntityLicenseTaskRequest());
  ...
  useEffect(() => {
    ...
    if (companyTypes != undefined) {
      let companyoptions = [];
      companyTypes.company_list.map((item) => {
    ...
    if (companyTypes != undefined) {
      let categoryoptions = [];
      companyTypes.Industry_list.map((item) => {
        categoryoptions.push({ value: item, label: item });
      });
  ...
  useEffect(() => {
    ...
    if (tempCoCompany != undefined) {
      ...
      tempCoCompany.forEach((element) => {
        let tempLicenseArr = [];
        element.license.map((item) => tempLicenseArr.push(item));
        // console.log("go this licese",tempLicenseArr)
        let obj = {
          ...element,
          coUserID: element.company_docname,
          selectedCompany:
            element.company_docname != undefined
              ? { value: element.company_docname, label: element.company_type }
              : null,
          selectedCategory:
            element.business_category != undefined
              ? { value: element.business_category, label: element.business_category }
              : null,
          company_name: element.company_name,
          company_country: element.company_country,
          company_pincode: Number(element.company_pincode),
          companyNameError: "",
          isExist: true,
          isEdited: false,
  ...
  const openChooseLicenseModel = (type, index, item) => {
    ...
    let FieldObj = {
      category: selectedObj.industry_type,
  ...
  const handelChange = (e, name, index, item) => {
    ...
    if (name === "companyName") {
      ...
      companyList[index].full_name = e.UserName;
    ...
    setTimeout(() => {
      ...
      if (
        companyList[index].company_name != "" &&
  ...
  const close = (fieldData) => {
    ...
    if (isSameLicenses === false || isSameLicenses === undefined) {
      ...
      if (
        ...
        tempCoCompany[selectedIndex].company_name ===
          ...
          <div id="drawerChild" className="sideBarFixed">
            {licenseModalHideShow && (
                ...
                //   addLicense={addLicense}
                  index={selectedIndex}
                  closeDrawer={(data, action) => close(data, action)}
                />
              </>
            )}             
          </div>
