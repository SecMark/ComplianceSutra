...
function CoManagment({handleClose}) {
    ...
    useEffect(() => {
        const companyTypes = state && state.taskReport && state.taskReport.companyTypeInfo
            && state.taskReport.companyTypeInfo.CompanyInfo && state.taskReport.companyTypeInfo.CompanyInfo[0]
            && state.taskReport.companyTypeInfo.CompanyInfo[0][0] && state.taskReport.companyTypeInfo.CompanyInfo[0][0].CompanyType

        const categoryTypes = state && state.taskReport && state.taskReport.companyTypeInfo
            && state.taskReport.companyTypeInfo.CompanyInfo && state.taskReport.companyTypeInfo.CompanyInfo[2]
            && state.taskReport.companyTypeInfo.CompanyInfo[2][0] && state.taskReport.companyTypeInfo.CompanyInfo[2][0].Categories
    ...
    useEffect(() => {
        let tempCoCompany = state && state.taskReport
            && state.taskReport.coEntityLicenseTask && state.taskReport.coEntityLicenseTask.entityLicenseInfo
        if (tempCoCompany != undefined) {
            ...
            tempCoCompany.forEach(element => {
                ...
                let obj = {
                    ...
                    selectedCompany: element.EntityTypeID != undefined ? { value: element.EntityTypeID, label: element.EntityType } : null,
                    selectedCategory: element.Category != undefined ? { value: element.Category, label: element.Category } : null,
                    companyNameError: "",
                    isExist: true,
                    isEdited: false,
    ...
    useEffect(() => {
        const tempUsers = state && state.taskReport &&
            state.taskReport.getUserByRole && state.taskReport.getUserByRole.getUserByRole
    ...
    useEffect(() => {
        let addEditStatus = state && state.taskReport && state.taskReport.CompanyAddEditStatus &&
    ...
    useEffect(() => {
        let deleteStatus = state && state.taskReport && state.taskReport.companyDeleteStatus &&
    ...
    const handelChange = (e, name, index, item) => {
        ...
        if (name === "companyName") {
            ...
            if (item.isExist) {
                if (selectedCompany != undefined && companyList[index].Category === selectedCompany.Category) {
                    companyList[index].selectedLicenseArray = selectedCompany.selectedLicenseArray
                    if (companyList[index].EntityTypeID === selectedCompany.EntityTypeID &&
    ...
    const validateExistingName = (e, index) => {
        if (e.target.value != selectedCompany.EntityName && e.target.value != "") {
            ...
            const isSameLicenses = checkWithPreviousLicenses(selectedCompany.selectedLicenseArray, tempCoCompany[index].selectedLicenseArray)
            tempCoCompany[index].EntityName = selectedCompany.EntityName
            tempCoCompany[index].isEdited = false

            if (tempCoCompany[index].EntityTypeID === selectedCompany.EntityTypeID &&
    ...
    const validateCompanyName = (e, index) => {
        if (e.target.value != "") {
            let payload = { loginID: e.target.value.trim(), pwd: "", rememberme: 0, loginty: "AdminCompany" };
            let companyList = [...companyDetails]
            api.post("/api/availabilityCheck", payload).then(function (response) {
    ...
    const close = (fieldData) => {
        ...
        let tempCoCompany = [...companyDetails]
        const isSameLicenses = checkWithPreviousLicenses(selectedCompany.selectedLicenseArray, fieldData.selectedLiecenseIdArray)
        if (isSameLicenses === false || isSameLicenses === undefined) {
            tempCoCompany[selectedIndex].selectedLicenseArray = fieldData.selectedLiecenseIdArray
            setCompanyDetails(tempCoCompany)
        } else {
            if (tempCoCompany[selectedIndex].EntityTypeID === selectedCompany.EntityTypeID &&
        ...
        return (
            ...
            >
                <div className="model-design-delete-company big-height-company">
                    ...
                    {currentCompanyName}&nbsp;? All selected licenses and assigned tasks will be deleted.
                 </div>
                    <div className="last-two-model-btn">
                        <button onClick={() => handleDeleteClick(null, 3)} className="btn cancel-delete">CANCEL</button>
                        <button onClick={() => handleDeleteClick(selectedCompany, 2)} className="btn delete-Record">DELETE</button>
                    </div>
                </div>
            </Modal>)
    }
    ...
    const handleSaveChanges = (index) => {
        ...
        const licenseIDgrpStr = selectedLiecenseList.length > 0 ? selectedLiecenseList.join(",") : "";

        const payload = {
            licenseSubID: 0,
            entityId: tempCoCompany && tempCoCompany[index] && tempCoCompany[index].isExist ? tempCoCompany[index].EntityId : 0,
        ...
        dispatch(
            coActions.insCertificateDetailsRequest(payload)
        );
        setShowAdd(false);
    ...
    const checkButtonDisabledColor = (key) => {
        ...
        list && list.map((item, index) => {
           if (index === key) {
              if (item.EntityName === "" ||
                 item.Category === "" || item.companyNameError != "") {
                 isNext = false
                 return isNext;
              }
           }
            ...
            { <div className="company-detail-mobile-box">
                    ...
                    <div class="input-box-mobile">
                        <input
                            ...
                            onBlur={(e) => { !item.isExist ? validateCompanyName(e, index) : validateExistingName(e, index) }}
                            onChange={(e) => (selectedIndex === undefined || selectedIndex === index) ? handelChange(e, "companyName", index, item) : true}
                        />
                        </div>
                        ...
                        <div className="flex mb-3">
                            ...
                            {item.CO && <div className="d-flex" style={{marginLeft:15}} onClick={() => (selectedIndex === undefined || selectedIndex === index) ? showBlock() : true}>
                                        <div class="login-assign-count-mobile">{getNameInitials(item.CO)}</div>
                                ...
                                <div id="drawerParent2" className="">
                                    ...
                                    <div className="col-2 col-sm-12 col-md-12 col-xl-12 d-block d-sm-none">
                                            <img className="close-icon-personal" src={closeBlack} alt="close Black" onClick={()=>{hideBlock()}}/>
                                        </div>
                        ...
                        {item.selectedLicenseArray.length > 0 && <div className="flex">
                            ...
                            <div className="col-9 pr-0 pl-0">
                                <div className="d-flex">
                                    <div style={{marginLeft:15}} class="assign-total-count-mobile" onClick={() => openChooseLicenseModel(2, index, item)}>{item.selectedLicenseArray.length > 0 ? item.selectedLicenseArray.length : ''}</div>
                                </div>                                
                            </div>
                        </div>}
                        <div class="input-box-mobile mb-0">
                            {item.selectedLicenseArray.length == 0 && <button 
                             onClick={() => openChooseLicenseModel(2, index, item)}
                            className={checkButtonDisabledColor(index) ? " btn add-license-mobile " : "btn add-license-mobile-co-company"}
                             disabled={checkButtonDisabledColor(index) ? false : true}>
                                add licenses
                                {item.EntityName === "" || item.EntityType === "" || item.Category === "" || item.companyNameError !== "" ?
                    ...
                    <div class="">
                            <div class="row align-right">
                                <div class="col-12 col-sm-12 col-md-12 col-xl-12 flex">
                                    <div class="cancel-link-mobile" onClick={() => handleUndoChanges(index)}>CANCEL</div>
                                    <button 
                                    className={ item.Category!== "" && item.EntityName!=="" && item.EntityType!=="" && item.selectedLicenseArray.length != 0 ? "btn mobile-save-company-blue" : "btn mobile-save-company" }
                                    disabled = {item.Category!== "" && item.EntityName!=="" && item.EntityType != "" && item.selectedLicenseArray.length != 0   ? false : true}
                                    onClick={() => handleSaveChanges(index)}>SAVE</button>
                                </div>
                            </div>
                    </div>
                    ...
                    <div class="d-flex">
                        ...
                        <div class="col-2 pr-0">
                            <div class="edit-delete">
                                <div class="">
                                    { companyDetails.length > 1 && <img className="" src={whiteDeleteIcon} alt="white Delete Icon" onClick={() => handleDeleteClick(item, 1)}/>}
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    return (
        ...
        <div className="co-personal-grid">
            {!isMobile && <div id="drawerParent" className="">
                <div id="drawerChild" className="sideBarFixed">
                    {licenseModalHideShow && <LicenseDrawer fields={fields}
            ...
            <div className="scroll-personal-grid d-none d-sm-block table-responsive">      
                <table className="settingCompanyTable table co-company-details-tbl table-responsive">
                    ...
                    <tbody>
                        {(companyDetails != undefined && companyDetails.length > 0 && companyTypes.length > 0 && categoryTypes.length > 0)
                            && companyDetails.map((item, index) => {
                                return (
                                    <>
                                        <tr className="focusRemove">
                                            <td className="companyName">
                                                {item.isExist && (!item.isEdited) && <div className="bk-Securities" onClick={() => (selectedIndex === undefined || selectedIndex === index) ? handleOnNameClick(index, item) : true} >{item.EntityName}</div>}
                                                {(!item.isExist || (item.isExist && item.isEdited)) &&
                                                    <>
                                                        <input
                                                            ...
                                                            onChange={(e) => (selectedIndex === undefined || selectedIndex === index) ? handelChange(e, "companyName", index, item) : true}
                                                        />
                                                        {item.companyNameError != "" && <p className="input-error-message">{item.companyNameError}</p>}
                                                    </>}
                                            </td>
                                            ...
                                            <td>
                                                {item.coUserID !== undefined &&
                                                    <div className={"holding-list-bold-title-background " + (coHoveredIndex === index ? 'activeName' : '')}
                                                        onClick={() => (selectedIndex === undefined || selectedIndex === index) ? setAssignPromptIndex(index) : true}
                                                        onMouseOver={() => setCoHoveredIndex(index)}
                                                        onMouseOut={() => setCoHoveredIndex(undefined)}>
                                                        <span className="circle-dp">{getNameInitials(item.CO)}</span> {item.CO} </div>}
                                                {!item.isExist && item.coUserID === undefined && <div className="assign-with-icon" onClick={() => (selectedIndex === undefined || selectedIndex === index) ? setAssignPromptIndex(index) : true}><img className="delete-Icon-check" src={assignIconCircle} alt="check Icon" /> assign </div>}
                                            ...
                                            <td>
                                                <div className="license-count-lable">
                                                    <div>{item.selectedLicenseArray.length > 0 ? item.selectedLicenseArray.length : ''}</div>
                                                    {item.selectedLicenseArray.length > 0 && <button id={'editLicense' + index} className="btn buttonprimary" onClick={() => openChooseLicenseModel(2, index, item)}>edit licenses</button>}
                                                    {item.selectedLicenseArray.length <= 0 && <button id={'addLicense' + index} className="btn buttonprimarygray" onClick={() => openChooseLicenseModel(1, index, item)}>Add Licenses</button>}
                                                </div>

                                            </td>
                                            <td className="deleteIconCheck">
                                                {item.isExist && selectedIndex != index && companyDetails.length > 1 && <img className="delete-Icon-personal" src={blackDeleteIcon} alt="company Drop Arrow Icon" onClick={() => handleDeleteClick(item, 1)} />}
                                                {selectedIndex === index &&
                                                    <div>
                                                        <img className="delete-Icon-check" src={item.Category!== "" && item.EntityName!=="" && item.EntityType!=="" && item.selectedLicenseArray.length != 0 ? greenCheck : grayCheck } alt="check Icon" onClick={() =>{item.Category!== "" && item.EntityName!=="" && item.EntityTypeID != 0 && item.selectedLicenseArray.length != 0 && handleSaveChanges(index)}} />
                                                        <img className="delete-Icon-check" src={redCheck} alt="delete Icon" onClick={() => handleUndoChanges(index)} />
                                                    </div>
                                                }
                                            </td>
                                        </tr>
