               ...
               <div className="row">
                  ...
                  <div className="col-8 col-sm-7 col-md-7 col-xl-7">
                     <div className="Filter-drop-down">
                        <ul>
                           <li><p className="filter-by d-none d-sm-block">Filter by:</p></li>
                           <li><Dropdown className="d-none d-sm-block" arrowClosed={<span className="arrow-closed d-none d-sm-block" />} onChange={onCategoryChange}
                              arrowOpen={<span className="arrow-open" />} options={options} value={selectedCategory} placeholder="Select an option" /></li>
                           <li><span className="dots-div dropIcon d-block d-sm-none" ref={innnerDropdown}>
                              {
                                 showFilter ?
                                    <img src={mobileDropDown} onClick={filterDropDown} style={{ height: "28px" }} />
                                    :
                                    <img src={filterIcon} onClick={filterDropDown} style={{ height: "18px", marginRight: "5px" }} />

                              }
                              {/* <li><span className="dots-div dropIcon d-block d-sm-none" ref={innnerDropdown}><img src={mobileDropDown} onClick={filterDropDown} /> */}
                              {showFilter && <div className="drop-div top-pt">
                                 {options.map((ele) => {
                                    return (
                                       <p className="dots-option" value={ele.value} onClick={() => onCategoryChange(ele)}>{ele.label}</p>
                                    )
                                 })}
                              </div>}
                           </span>
