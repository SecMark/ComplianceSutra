...
function BoardView({ setCurrentBoardViewBy, currentBoardViewBy }) {
    ...
    useEffect(() => {
        if (currentBoardViewBy !== "status") {
            let data = taskList && taskList.length > 0 && taskList.sort((a, b) => a.Status.localeCompare(b.Status))
            setTaskData(data)
        } else {
    ...
    const data1 = () => {
        ...
        if (taskData) {
            for (i = 0; i < taskData.length; i++) {
                ...
                if (obj && obj.length > 0 && obj.some(i => !i.LicenseCode.includes('Norec'))) {
                    for (j = 0; j < taskData[i].Details.length; j++) {
                        obj = taskData[i] && taskData[i].Details && taskData[i].Details.map((item, index) => {
