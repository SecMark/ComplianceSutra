...
function Governance({ history }) {
  ...
  // const [assignTaskEmail, setAssignTaskEmail] = useState([{ email: "", companyName: "", categoryName: "", emailAvail: "" }]);
  ...
  const checkEmailAlreadyExistsOrNot = async (email, index, role) => {
    ...
    if (role === "approval" && isEmail(email)) {
      ...
      if (coEmails === undefined) {
        let stausEmails = statusReportEmail.find((item) => item.email === email)
  ...
  const onDoneButtonClick = () => {
    ...
    assignTaskEmail &&
      ...
      assignTaskEmail.map((item, index) => {
        let compObj = []
        if (item.companyName !== "") {
          // complianceOfficerMailList.push(item.companyName);
          compObj.push(item.companyName)
        }
        if (item.email !== "") {
          // complianceOfficerMailList.push(item.email);
      ...
      <div key={index} className="col-12 row">
        ...
        <div className="col-4 col-md-4 col-sm-4 col-xl-4">
          <div className="input-grid-right">
            ...
            {(assignButtonIndex.length !== 0 && !companyExists(`${item.EntityName}${index}`) ? true : assignButtonIndex.length === 0 ? true : false) && (
              <span
                onClick={(e) => {
                  const values = [...assignButtonIndex];
                  let obj = { EntityName: `${item.EntityName}${index}` };
                  values.push(obj)
                  setAssignButtonIndex(values);
                }}
                className="assign-link d-block d-sm-none">Assign</span>)}
          </div> */}
            {/* {assignButtonIndex && assignButtonIndex.length > 0 && companyExists(`${item.EntityName}${index}`) && (
            <div className="form-group mt-2 d-none d-sm-block">
              <div>
                <input type="text"
                  className="form-control place-style"
                  placeholder="Add email to assign"
                  name="assigntaskemail"
                  autoComplete="off"
                  onBlur={(e) => checkEmailAvailorNot(e, index)}
                  onChange={(e) => { handleAssignTaskEmail(e, index, item) }}
                  value={item && item[index] && item[index].email}
                />
                {assignTaskEmail && assignTaskEmail[index] && assignTaskEmail[index].email !== "" && !isEmail(assignTaskEmail[index].email) && (
                  <p className="input-error-message">Please Enter Valid Email Address</p>)}
                {assignTaskEmail && assignTaskEmail[index] && assignTaskEmail[index].emailAvail !== "" && (
                  <p className="input-error-message">Email is alreday assigned to other role</p>)}
              </div>
            </div>
          )} */}
            ...
            <div className="bottom-input">
              ...
              <div className="bottom-logo-strip-goverance">
                <div className="who-assign-task">
                  ...
                  <div className="col-10 pl-0 pr-0"><div class="border-line"></div></div>
