...
function AssignDrawerMobile(props) {
    ...
    const checkEmailAlreadyExistsOrNot = async (email) => {
        ...
        if (isEmail(email)) {
            await
                axios.post(`${BACKEND_BASE_URL}/api/availabilityCheck`, {
                    loginID: email.trim(),
                    pwd: "",
                    rememberme: 0,
                    loginty: "AdminEmail",
            ...
            <div className="container-fluid pl-0 pr-0">
                <div className="choose-licenses">
                    ...
                    <div className="choose-licenses-title choose-licene-goverance">
                        ...
                        <div className="input-comment-box">
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    if (props.comeFrom === "governance" && isEmail(e.target.value)) {
                            ...
                            <div className="inputIcon"
                                onClick={() => {
                                    if (props.comeFrom === "assignTask" && emailVal != "") {
                                        closeButtonCall(1)
                                    } else if (props.comeFrom === "governance" && emailVal != "" && !isEmailExist) {
                                        closeButtonCall(1)
                                    }
                                }}
                                style={{
                                    opacity: props.comeFrom === "assignTask" && isEmail(emailVal) && emailVal != "" ? 1
                                        : props.comeFrom === "governance" && isEmail(emailVal) && emailVal != "" && (!isEmailExist) ? 1 : 0.6
                                }}>
                                <img src={inputRightArrow} alt="" style={{ cursor: "pointer" }} />
                            </div>
                        ...
                        {emailErr === "" && emailListHideShow && props.comeFrom === "assignTask" &&
                            <div className="dropdown-email">
                                {props.emailList && props.emailList.length > 0 && (
                                    <div className="dropdown-user-list">
                                        {props.emailList && props.emailList.length > 0 && props.emailList.map((data, id) => (
                                            <div key={id} className="user-list-row">
                                                <div onClick={() => { setEmailVal(data.email); setEmailListHideShow(false) }}
                                        ...
                                        <div>
                                            ...
                                            <div className="user-list-row">
                                                <img className="blue-icon" src={dropdownBlueIcon} alt="dropdown Blue Icon" />
                                                <div className="SecMark-Expert-Review">CAPMTech Expert Review</div>
                                                <div style={{ cursor: "pointer" }} onClick={() => props.openExpertReview()} className="dropDown-blueLink">View Details</div>
                                            </div>
                                        </div>
                                    </div>)}
                            </div>}
